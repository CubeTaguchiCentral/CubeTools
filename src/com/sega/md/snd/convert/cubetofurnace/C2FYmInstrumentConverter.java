...
public class C2FYmInstrumentConverter {
    ...
    public static Feature convertCubeInstrumentToFurnaceFeature(byte[] cubeFmInstrument, boolean ssgEg){
        ...
        data[4+0*8+1] = ((ALGO_SLOTS[algo]&0b0001)>0)?(byte)(0x7F-(cubeFmInstrument[1*4+0]&0x7F)):cubeFmInstrument[1*4+0];
        ...
        data[4+1*8+1] = ((ALGO_SLOTS[algo]&0b0010)>0)?(byte)(0x7F-(cubeFmInstrument[1*4+1]&0x7F)):cubeFmInstrument[1*4+1];
        ...
        data[4+2*8+1] = ((ALGO_SLOTS[algo]&0b0100)>0)?(byte)(0x7F-(cubeFmInstrument[1*4+2]&0x7F)):cubeFmInstrument[1*4+2];
        ...
        data[4+3*8+1] = ((ALGO_SLOTS[algo]&0b1000)>0)?(byte)(0x7F-(cubeFmInstrument[1*4+3]&0x7F)):cubeFmInstrument[1*4+3];
